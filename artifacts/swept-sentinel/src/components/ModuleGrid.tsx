...
export default function ModuleGrid({ modules, isLoading, onExecute, batchMode, selectedIds = new Set(), onToggleSelect }: ModuleGridProps) {
  if (isLoading) {
    return (
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-3">
      {modules.map(m => {
        ...
        return (
          <Button
            key={m.id}
            data-testid={`module-btn-${m.id}`}
            variant="outline"
            className={`h-20 py-3 px-4 flex flex-col items-start justify-center text-left font-mono rounded-none transition-all duration-200 group relative overflow-hidden shadow-sm
              ${batchMode
                ? isSelected
                  ? "bg-primary/20 border-primary text-primary"
                  : "bg-card border-border hover:bg-primary/10 hover:border-primary hover:text-primary"
                : "bg-card border-border hover:bg-primary/10 hover:border-primary hover:text-primary"
              }`}
            onClick={() => batchMode ? onToggleSelect(m.id) : onExecute(m.id)}
          >
            <div className="absolute inset-0 bg-gradient-to-r from-primary/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity"></div>
            {batchMode && isSelected && (
              <div className="absolute top-1 right-2 text-primary text-xs font-bold">✓</div>
            )}

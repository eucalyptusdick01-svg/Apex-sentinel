...
- Real-data modules implemented so far (IDs 1–30, 51, 52, 54, 71, 92–103, 151–161, 201–215, 230 — 72 total): IP Tracker/Geolocate/Proxy Check (ip-api.com), DNS Resolve (Node `dns/promises`), Port Scan (raw TCP connect scan), Whois Query (RDAP via rdap.org), Email Rep (DNS MX + Kickbox), DB Search/News Search/People Search/Image Search/Site Enum (SerpApi Google — requires SERPAPI_KEY), GitHub Lookup (api.github.com), Username Check (GitHub + Reddit), SSL Cert Info (raw TLS), Wayback Check (archive.org), HTTP Fingerprint (headers), VIN Check (NHTSA free API), CVE Lookup (NVD/NIST free API), MAC Lookup (api.macvendors.com), BGP Route (ipinfo.io — see Gotchas), CDN Origin (DNS CNAME + HTTP headers), DMARC Analyze (DNS TXT), Subdomain Scan (HackerTarget hostsearch), Reverse IP (HackerTarget reverseiplookup), Tech Stack (header + source fingerprinting), Admin Finder (HTTP probe 27 paths), Robots Scan (robots.txt + sitemap.xml), API Probe (29 common API/doc endpoints), Cert History (crt.sh certificate transparency — note: slow for large domains), Shodan Probe (Shodan InternetDB — free no-key, open ports/vulns), Threat Intel (GreyNoise Community — free no-key, scanner classification), Tor Check (Tor Project exit node list), URL Scan (URLScan.io public search), CORS Check (OPTIONS preflight + reflected-origin analysis), CSP Analyze (Content-Security-Policy parser), REST Probe (HTTP request/response inspector), Entropy Calc (Shannon entropy + byte distribution), String Extract (printable ASCII/UTF-16 extraction), File Ident (magic-byte database + extension lookup), AES Cipher (AES-256-CBC / PBKDF2 encrypt-decrypt), RSA Keygen (2048/4096 keypair + OAEP encrypt-decrypt), Passphrase Gen (EFF wordlist + PIN/hex modes), HMAC Calc (SHA256/512/SHA1/BLAKE2), Hash Compare (MD5/SHA1/SHA256/SHA512/CRC32 + verify mode), Regex Test (full Python re engine + built-in patterns), Cron Parser (expression explainer + next fire times), TZ Convert (IANA tz database, all timezones), Rand Data (uuid/ip/ipv6/mac/email/name/card/password/jwt/etc), Pass Audit (entropy + pattern heuristics + crack-time estimate), Token Count (GPT token estimator + readability + model limit check), Evidence Hash (chain-of-custody MD5/SHA1/SHA256/SHA512 record), QR Encode (ASCII terminal QR code), Code Stats (line counts + language detection + complexity).
...
- 72 modules currently return real, live data (free APIs/protocols or local computation). The remainder are simulated placeholders pending paid/authenticated data sources.

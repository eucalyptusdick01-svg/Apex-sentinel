#!/usr/bin/env python3
"""
sweptsentinel.py — CLI for the Swept Sentinel recon dashboard

Usage:
  python sweptsentinel.py list                          # list all 230 modules
  python sweptsentinel.py list network                  # list a category
  python sweptsentinel.py run <module_id> <target>      # run a module
  python sweptsentinel.py search <keyword>              # search modules by name

Examples:
  python sweptsentinel.py run 1 8.8.8.8
  python sweptsentinel.py run 3 google.com
  python sweptsentinel.py run 11 torvalds
  python sweptsentinel.py list recon
  python sweptsentinel.py search cert
"""
...
CAT_COLOUR = {
    ...
    "RECON":    "\033[93m",   # yellow
    "EXPLOIT":  "\033[91m",   # red
    "INTEL":    "\033[95m",   # purple
    "ADVANCED": "\033[96m",   # cyan
}
                ...
                if buf.endswith(b"\n\n"):
                    ...
                    for raw in buf.split(b"\n"):
                        ...
                        if line.startswith("data:"):
                            value = line[5:].strip()
                            # SSE values are JSON-encoded strings — unwrap them
                            try:
...
def cmd_search(args):
    if not args:
        print(f"{RED}Usage: python sweptsentinel.py search <keyword>{R}")
        sys.exit(1)
...
def cmd_run(args):
    if len(args) < 2:
        print(f"{RED}Usage: python sweptsentinel.py run <module_id> <target>{R}")
        print(f"{DIM}Example: python sweptsentinel.py run 3 google.com{R}")
        sys.exit(1)
